    ...
    @foreach($locales as $locale)
        <div class="col-md" v-show="shouldShowLangGroup('{{ $locale }}')" v-cloak>
            <div class="form-group row align-items-center" :class="{'has-danger': errors.has('data_{{ $locale }}'), 'has-success': fields.data_{{ $locale }} && fields.data_{{ $locale }}.valid }">
                <label for="data_{{ $locale }}" class="col-md-2 col-form-label text-md-right">{{ trans('admin.customer.columns.data') }}</label>
                <div class="col-md-9" :class="{'col-xl-8': !isFormLocalized }">
                    <input type="text" v-model="form.data.{{ $locale }}" v-validate="'required'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('data_{{ $locale }}'), 'form-control-success': fields.data_{{ $locale }} && fields.data_{{ $locale }}.valid }" id="data_{{ $locale }}" name="data_{{ $locale }}" placeholder="{{ trans('admin.customer.columns.data') }}">
...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('status'), 'has-success': fields.status && fields.status.valid }">
    <label for="status" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.customer.columns.status') }}</label>
        <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" v-model="form.status" v-validate="'required'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('status'), 'form-control-success': fields.status && fields.status.valid}" id="status" name="status" placeholder="{{ trans('admin.customer.columns.status') }}">
